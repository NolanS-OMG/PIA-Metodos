const CalculoOsciladorAmortiguado = (masa, K, Y, F, x0, v0, t0) => {
    if ( ( (Y**2)/( 4 * (masa**2) ) ) === ( K/masa ) ) {
        const r = -Y/(2*masa);
        const x = ( Math.E ** ( r * t0 ) ) * ( v0 + ( r * v0 + (Y/masa) * x0 + (F/masa) ) * t0 ) + x_0;
        return x;
        ...
        const x = ( 1 / (r1 - r2) ) * ( ( v0 + (Y/masa)*x0 + (F/masa) ) * ( Math.E ** (r1*t0) - Math.E ** (r2*t0) ) + x0 * ( r1 * Math.E ** (r1*t0) - r2 * Math.E ** (r2*t0) ) );
        return x;
    } else {
        const w = Math.sqrt( ( K/masa ) - ( (Y**2)/( 4 * (masa**2) ) ) );
        const b = Y / (2 * masa);
        const x = ( Math.E ** (-b*t0) ) * ( ( (v0 + b*x0 + (F/masa)) / w )*Math.sin(w*t0) + x0*Math.cos(w*t0-Math.PI/2) ) + x_0;
        return x;
    }
}
...
const HabilitarDeshabilitar = (bool) => {
    ...
    FNumber.disabled = bool;
    FRange.disabled = bool;
    tiempoNumber.disabled = bool;
    tiempoRange.disabled = bool;
    botonInicio.disabled = bool;
}

const OsciladorAmortiguado = (masa, K, Y, F, t_iterado) => {
    if (t_iterado < tiempo_limite*1000) {
        ...
        if (paso) {
            ...
            setTimeout( () => {
                paso = true;
                let alturaResorte = CalculoOsciladorAmortiguado(masa, K, Y, F, x_0, 0, t_iterado/1000) * METROS_A_PIXELES;
                resorte.style.height = `${alturaResorte}px`;
                OsciladorAmortiguado(masa, K, Y, F, t_iterado + INTERVALOS_MILISEGUNDOS);
...
const IniciaSimulador = () => {
    paso = true;
    OsciladorAmortiguado(masaNumber.value, KNumber.value, YNumber.value, FNumber.value, 0);
}
...
const cambiaFRange = () => {
    FNumber.value = FRange.value;
}
const cambiaFNumber = () => {
    FRange.value = FNumber.value;
}
...
const FNumber = document.getElementById('fuerzaConstanteNumber');
const FRange = document.getElementById('fuerzaConstanteRange');
...
if ( FNumber && FRange ) {
    FRange.addEventListener('mouseup', cambiaFRange);
    FNumber.addEventListener('keyup', cambiaFNumber);
}
